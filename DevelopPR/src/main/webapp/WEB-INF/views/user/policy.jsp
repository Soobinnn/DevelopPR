...
var checkb = false;
$(function(){
	// 처음에 동의 활성화 끄기 및 체크 박스 해제
	  $('#agree_btn').prop("disabled", true).css({"background-color":"#aaaaa", "border":"2px solid #B9B9B9"});
	...
	$('#chk1').on('click',function()
	  {
			checka = $(this).prop('checked');
			onOff();
	  });
      $('#chk2').on('click',function(){
		 checkb = $(this).prop('checked');
		 onOff();
	  });
      ...
      $('#agree_btn').hover(function()
      {
    	  if(checka == true && checkb == true)
  		  {
    		  $('#agree_btn').css({"background-color":"#b9b9b9"});
  		  }
      },function(){
    	  if(checka == true && checkb == true)
  		  {
    		  $('#agree_btn').css({"background-color":"#585858", "border":"2px solid #585858", "color":"white"});
  		  }
      });
	...
	function onOff()
	{
		if(checka == true && checkb == true)
		{
			$('#agree_btn').prop("disabled", false).css({"background-color":"#585858", "border":"2px solid #585858", "color":"white"});			
		}
		else
		{
			 $('#agree_btn').prop("disabled", true).css({"background-color":"white", "border":"2px solid #B9B9B9", "color":"#B9B9B9"});
		}
	}
</script>
<title>이용약관</title>
		...
		<div class="container">
			<div class="area1" id="area1_1">
				...
				<input type="checkbox" id="chkAll" onclick=chkBox(this.checked)>
					<span id="chkAll1">동의</span>
				</label>
			</div>
			<div class="area1" id="area1_2">
				<label for="chk1"><span id="chk1_1">이용약관</span><span id="chk1_2">(필수)</span> <input type="checkbox" id="chk1"
					name="join"> <span id="chk1_3">동의</span>
				</label>
			</div>
			<div class="area2" id="area2_1">
				<p>제3장 계약 당사자의 의무</p>
				<p>제13조 회사의 의무</p>
				<p>1. “회사”는 관련 법령을 준수하고, 이 약관이 정하는 권리의 행사와 의무의 이행을 신의에 따라 성실하게 합니다.</p>
				<p>2. “회사”는 “이용자”가 안전하게 “서비스”를 이용할 수 있도록 개인정보 보호를 위해 보안 시스템을 갖추어야 하며 개인정보처리방침을 공시하고 준수합니다. “회사”는 이 약관 및 개인정보처리방침에서 정한 경우를 제외하고는 “이용자”의 개인정보가 제 3자에게 공개 또는 제공되지 않도록 합니다.</p>
				<p>3. “회사”는 계속적이고 안정적인 “서비스”의 제공을 위하여 “서비스”를 개선하던 중 설비에 장애가 생기거나 데이터 등이 멸실 된 때에는 천재지변, 비상사태, 현재의 기술로는 해결이 곤란한 기술적 결함 등 부득이한 사유가 없는 한 지체 없이 이를 수리 또는 복구하도록 최선의 노력을 다합니다.</p>
				<p>4. “이용자”가 “서비스”의 이용에 필요한 최소한의 기술사양을 갖추지 못한 경우 회사는 “서비스”를 제공할 의무가 없습니다. 제14조 이용자의 의무 1. “이용자는 다음 행위를 하여서는 안 됩니다. 
...
7) “회사”와 기타 제 3자의 저작권 등 지적재산권에 대한 침해 
8) “회사” 및 기타 제 3자의 명예를 손상시키거나 업무를 방해하는 행위 
...
11) 다른 “이용자”를 기망하여 부당한 이득을 얻는 행위 
12) “회사”가 제공하는 "서비스"를 사행성이 있는 방법 또는 기타 불건전한 방법으로 이용하는 행위 
13) 프로그램상의 버그를 악용하는 행위 
14) 권한 없이 회사의 클라이언트 프로그램을 변경하거나, 회사의 서버를 해킹하거나 웹사이트 또는 게시된 정보의 일부분 또는 전체를 임의로 변경하거나, 회사의 "서비스"를 비정상적인 방법으로 사용하는 행위 
15) “회사”의 동의 없이 영리, 영업, 광고, 정치활동 등을 목적으로 “서비스”를 사용하는 행위 
16) 1호 내지 15호의 행위를 유도하거나 광고하는 행위 
17) 기타 관련 법령에서 금지하거나 선량한 풍속 기타 사회통념상 허용되지 않는 행위 

2. “이용자”는 이 약관의 규정, 이용안내 및 “서비스”와 관련하여 공지한 주의사항, “회사”가 통지하는 사항 등을 확인하고 준수할 의무가 있습니다. 
3. “회사”는 제 1항, 제 2항 및 다음 각 호의 어느 하나에 해당하는 행위의 구체적인 유형을 “운영정책” 또는 “운영원칙”에서 정할 수 있으며, “이용자”는 이를 준수할 의무가 있습니다. 
1) “이용자”의 “계정(ID)”명, “캐릭터”명, “닉네임”명에 대한 제한 
2) 채팅내용과 방법에 대한 제한 
3) 게시판이용에 대한 제한 
4) “서비스”이용방법에 대한 제한
5) 기타 “이용자”의 “서비스” 이용에 대한 본질적 권리를 침해하지 않는 범위 내에서 “회사”가 “서비스” 운영상 필요하다고 인정되는 사항 

제4장 서비스 이용 제15조 서비스의 변경 및 내용수정 
1. “이용자”는 “회사”가 제공하는 “서비스”를 이 약관, 운영정책 및 “회사”가 설정한 서비스이용규칙에 따라 이용할 수 있습니다. 
2. “회사”는 “서비스” 내용의 제작, 변경, 유지, 보수에 관한 포괄적인 권한을 가집니다. 
3. “회사”는 “서비스”내 질서와 게임성, 기타 “서비스”의 원활한 유지를 위하여 필요한 조치를 취할 수 있습니다. 
4. “회사”가 상당한 이유가 있는 경우에 운영상, 기술상의 필요에 따라 “서비스”를 수정(패치) 할 수 있으며, “서비스”를 수정(패치) 하는 경우에는 해당 “서비스” 웹사이트 등을 통하여 공지합니다. 

제16조 서비스의 제공 및 중단 등 
1. “서비스”는 “회사”의 영업방침에 따라 정해진 시간 동안 제공됩니다. “회사”는 “서비스” 제공시간을 “서비스” 초기 화면이나 “홈페이지” 등에 적절한 방법으로 안내합니다. 2. “회사”는 원활한 “서비스” 환경 제공을 위해 백그라운드 데이터 기능을 제공합니다. 백그라운드 데이터 기능이란, 애플리케이션이 백그라운드(실행중) 상태에서 데이터를 통신하는 것을 의미하며, 이러한 백그라운드 데이터 기능 이용 시 데이터 요금이 발생될 수 있습니다. 3. 제 1항에도 불구하고, 다음 각 호의 어느 하나에 해당하는 경우에는 일정한 시간 동안 “서비스”가 제공되지 아니할 수 있으며, 해당 시간 동안 “회사”는 “서비스”를 제공할 의무가 없습니다. 
1) 컴퓨터 등 정보통신설비의 보수점검, 교체, 정기점검 또는 게임 내용이나 “서비스”의 수정을 위하여 필요한 경우 
2) 해킹 등의 전자적 침해사고, 통신사고, “이용자”들의 비정상적인 이용행태, 미처 예상하지 못한 “서비스”의 불안정성에 대응하기 위하여 필요한 경우 
...
4. “회사”는 제 3항 제 1호의 경우, 매주 또는 격주 단위로 일정 시간을 정하여 “서비스”를 중지할 수 있습니다. 이 경우 “회사”는 최소한 24시간 전에 그 사실을 “이용자”에게 “서비스” 초기 화면이나 “홈페이지” 등에 고지합니다. 
5. 제 3항 제 2호의 경우, “회사”는 사전 고지 없이 “서비스”를 일시 중지할 수 있습니다. “회사”는 이러한 경우 그 사실을 “서비스” 초기 화면이나 “홈페이지” 등에 사후 고지할 수 있습니다. 
6. “회사”는 “회사”가 제공하는 “무료서비스” (“이용자”가 별도 비용지급 없이 이용할 수 있는 “서비스”를 의미하며, 이하 같음) 이용과 관련하여 이용자에게 발생한 어떠한 손해에 대해서도 책임을 지지 않습니다. 다만, “회사”의 고의 또는 중대한 과실로 인하여 발생한 손해의 경우는 제외합니다. 
7. “회사”는 “회사”가 제공하는 "유료서비스"의 중지·장애와 관련하여 다음 각 호에 따라 보상하며, 이용자는 “회사”에 대하여 별도의 손해배상을 청구할 수 없습니다. 

			</div>
			<div class="area1" id="area1_3">
				<label for="chk2"><span id="chk2_1">개인정보 수집 및 이용</span><span id="chk2_2">(필수)</span> <input type="checkbox"
					id="chk2" name="join"> <span id="chk2_3">동의</span>
				</label>
			</div>
			<div class="area3" id="area3_1">
