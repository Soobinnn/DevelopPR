  ...
  $(function(){
	...
	$('#userName').blur(function()
	{
		...
		if(!userNameCheck.test($('#userName').val()))
		{
			...
			$('#nameCheckMsg').text('이름 형식에 맞게 입력해주세요.(한글2~4자)').css("color","red");
			CheckName = false;
		}
		else	
		{
	...
	$('#userPw').blur(function()
	{
		...
		if(!userPwCheck.test($('#userPw').val()))
		{
			...
			$('#pwCheckMsg').text('비밀번호 형식에 맞게 입력해주세요.(최소8자 이상, 문자,숫자,특수문자 최소 1자)').css("color","red");
			$('#userPw').focus();
			CheckPw = false;
		}
		else	
			...
			$.ajax({
				...
				success : function(data)
				{
					...
					{		
						...
						/* $('#userEmail').focus(); */
						$('#nickCheckMsg').text('닉네임 형식에 맞게 입력해주세요.(2~20자)').css("color","red");
						CheckNick = false;
					}
					else 
					{
	...
	$('#userPhone').blur(function()
	{
		...
		if(!userPhoneCheck.test($('#userPhone').val()))
		{
			...
			$('#phoneCheckMsg').text('휴대폰번호 형식에 맞게 입력해주세요.(숫자만 입력)').css("color","red");
			CheckPhone = false;
		}
		else
		{
            ...
            <div id="middle">
               <form id="joinForm_form" action="${path}/user/joining" name="form1" method="post" onsubmit="return checks()">
                  <ul id="user_ul">
                     ...
                     <li class="user_li" id="user_li_email"><input id="userEmail" type="text" autocomplete="off"
                     ...
                     <li class="user_li"><input id="userName" type="text" autocomplete="off" value=""
                        placeholder=" 이름" name="userName">
                        <span id="nameCheckMsg"></span>
                     </li>
                     <li class="user_li"><input id="userPw" type="password" value=""
                        placeholder=" 비밀번호" name="userPw">
                         <span id="pwCheckMsg"></span>
                        <p id="p3">비밀번호는 ~ 영문, 숫자, 특수문자를 조합해 주세요.</p></li>
                     <li class="user_li"><input id="userPwChk" type="password" value=""
                        placeholder=" 비밀번호 확인">
                         <span id="pwchCheckMsg"></span>
                        <p id="p4">비밀번호를 다시 한번 입력해 주세요.</p></li>
                     <li class="user_li"><input id="userNick" type="text" autocomplete="off"
                        value="" placeholder=" 닉네임" name="userNick">
                         <span id="nickCheckMsg"></span>
                        <p id="p5">사용하실 닉네임을 2자~20자 한글, 영문, 숫자를 사용해 입력해주세요.</p>
                    </li>
                    <li class="user_li"><input id="userPhone" type="text" autocomplete="off"
                        value="" placeholder=" 핸드폰" name="userPhone">
                        <span id="phoneCheckMsg"></span>
                        <p id="p6">핸드폰 번호는 '-'를 제외하고 입력해 주세요.</p>
                    </li>
